    ...
    public TileGrid() {
        ...
        for (int i = 0; i < map.length; i++){
            for (int j = 0; j < map[i].length; j++){
                map[i][j] = new Tile(i * TILE_SIZE, j * TILE_SIZE, TILE_SIZE, TILE_SIZE, TileType.FreshGrass);
            }
        }
    }
            ...
            for (int j = 0; j < map[i].length; j++){
                switch (newMap[j][i]) {
                    case 0:
                        ...
                        break;
                    case 1:
                        map[i][j] = new Tile(i * TILE_SIZE, j * TILE_SIZE, TILE_SIZE, TILE_SIZE, TileType.Route);
                        break;
                    case 2:
